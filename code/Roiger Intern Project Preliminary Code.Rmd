...
specrich <- ddply(idsmall, ~ uniquePlotIDs + domainid + date + Year + HoursOfTrapping + Precipmm + Max.TempC, summarize, num_species = length(scientificName))
...
ggplot(specrich, aes(Max.TempC, num_species))+
  geom_point(aes(color = Continental), size = 2)+
...
tempplot2016 <-ggplot(tars2016, aes(as.Date(date), as.numeric(Max.TempC)))+
  geom_point(color = "light green", size = 2)+
