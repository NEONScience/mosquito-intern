...
trap = read.csv('N:/Science/FSU/Intern Projects/2017_CharlotteRoiger/mos_trapping_in.csv')
sort = read.csv('N:/Science/FSU/Intern Projects/2017_CharlotteRoiger/mos_sorting_in.csv')
id = read.csv('N:/Science/FSU/Intern Projects/2017_CharlotteRoiger/mos_identification_in.csv')
pathogen = read.csv('N:/Science/FSU/Intern Projects/2017_CharlotteRoiger/mos_pathogenresults_in.csv')
taxonomy = read.csv("~/GitHub/mosquito-intern/resources/mosquito_taxonomy.csv")
precip.df = read.csv('~/GitHub/mosquito-intern/resources/precipitation.csv')
temp.df = read.csv("~/GitHub/mosquito-intern/resources/temperature_maximum.csv")
domain.df = read.csv("~/GitHub/mosquito-intern/data/domainid_siteid_match.csv")
...
id$HoursOfTrapping<-ifelse(substr(id$setDate,9,10)!=substr(id$collectDate,9,10), 24 - ((as.numeric(substr(id$setDate,15,16))/60)+(as.numeric(substr(id$setDate,12,13)))) + ((as.numeric(substr(id$collectDate,15,16))/60)+(as.numeric(substr(id$collectDate,12,13)))),((as.numeric(substr(id$collectDate[8],15,16))/60)+(as.numeric(substr(id$collectDate[8],12,13)))) - ((as.numeric(substr(id$setDate[8],15,16))/60)+(as.numeric(substr(id$setDate[8],12,13)))))
...
specrich <- ddply(idsmall, ~ uniquePlotIDs + date + Year + HoursOfTrapping + Precipmm + Max.TempC, summarize, num_species = length(scientificName))
...
ggplot(specrich, aes( HoursOfTrapping, num_species))+
  geom_point()+
  labs(x = "Hours of Trapping", y = "Number of Species")+
  ggtitle("Species Richness by Hours of Trapping")
...
ggplot(specrich, aes(Max.TempC, num_species))+
  geom_point(aes(color = lat2))+
  labs(x = "Maximum Temperature in Celsius", y = "Number of Species")+
  ggtitle("Species Richness by Maximum Temperature")

ggplot(specrich, aes(Precipmm, num_species))+
  geom_point(aes(color = lat2))+
  labs(x = "Precipitation in Millimeters", y = "Number of Species")+
...
tarssmall <- merge(x = tarstax.df, y = tarssmall, by.x = "domainID", by.y = "domainid")

tarssmall$lat2<- as.numeric(tarssmall$lat2)
tarssmall$lon2<- as.numeric(tarssmall$lon2)
...
tarssmall <- tarssmall[order(as.Date(tarssmall$date)),]

#Subsets containing only the rows where Cu. tarsalis was first sampled in each geographic subdivision
tars.firstdomain <- tarssmall[match(unique(tarssmall$domainID), tarssmall$domainID),]
tars.firstsite <- tarssmall[match(unique(tarssmall$siteid), tarssmall$siteid),]
tars.firstplot <- tarssmall[match(unique(tarssmall$uniquePlotIDs), tarssmall$uniquePlotIDs),]
...
plot2016 <-ggplot(tars2016, aes(as.Date(date), as.numeric(individualCount)))+
  geom_point(color = "darkblue")+
  labs(x = "Date", y = "Count of Culex tarsalis")+
  scale_x_date(labels = date_format("%m-%Y"), date_breaks = "1 month")+
  ggtitle("Date versus Count of Culex tarsalis for 2016")
plot2014 <-ggplot(tars2014, aes(as.Date(date), as.numeric(individualCount)))+
  geom_point(color = "pink")+
  scale_x_date(labels = date_format("%m-%Y"), date_breaks = "1 month", limits = as.Date(c("2014-05-01","2014-11-01")))+
  labs(x = "Date", y = "Count of Culex tarsalis")+
  ggtitle("Date versus Count of Culex tarsalis for 2014")

grid.arrange(plot2014,plot2016)

tempplot2016 <-ggplot(tars2016, aes(as.Date(date), as.numeric(Max.TempC)))+
  geom_point(color = "light green")+
  labs(x = "Date", y = "Maximum Temperature in C")+
  scale_x_date(labels = date_format("%m-%Y"), date_breaks = "1 month")+
  ggtitle("Date versus Maximum Temperature for 2016")
tempplot2014 <-ggplot(tars2014, aes(as.Date(date), as.numeric(Max.TempC)))+
  geom_point(color = "purple")+
  scale_x_date(labels = date_format("%m-%Y"), date_breaks = "1 month", limits = as.Date(c("2014-05-01","2014-11-01")))+
  labs(x = "Date", y = "Maximum Temperature in C")+
  ggtitle("Date versus Maximum Temperature for 2014")

grid.arrange(tempplot2014, tempplot2016)

grid.arrange(tempplot2016, plot2016)

q <-ggplot(tarssmall, aes(as.numeric(lat2), as.Date(date), frame = date))+
      geom_point(aes(color= Year))+
      scale_y_date(labels = date_format("%m-%Y"), date_breaks = "1 month", limits = as.Date(c("2014-05-01","2016-11-01")))+
      scale_x_continuous(limits = c(30,65))+
      ggtitle("Date versus Latitude for 2016")

# gg + geom_point(data = tars.firstdomain, aes(lon2,lat2, fill = domainID))
