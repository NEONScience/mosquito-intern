...
specrich <- ddply(idsmall, ~ uniquePlotIDs + domainid + siteid + date + Year + HoursOfTrapping + Precipmm + Max.TempC, summarize, num_species = length(scientificName))
...
#debug(Templag)
TwoWeekAvgMaxTemp<-mapply(Templag, specrich$siteid, specrich$date)
#undebug(Templag)

end <- Sys.time(); end - st # profiling end time; calculate function execution duration

specrich <- cbind(specrich, TwoWeekAvgMaxTemp)

#Two week average precip
# st <- Sys.time()

TwoWeekAvgMaxPrecip<-mapply(Preciplag, specrich$siteid, specrich$date)

# end <- Sys.time(); end-st

specrich <-cbind(specrich, TwoWeekAvgMaxPrecip)
...
ggplot(specrich, aes(TwoWeekAvgMaxTemp, num_species))+
...
tars2014 <- tarssmall[tarssmall$Year==2014,]
tars2016 <- tarssmall[tarssmall$Year==2016,]
...
tarssmall$TwoWeekAverageT<- mapply(Templag, tarssmall$siteid, tarssmall$date)

tarssmall$TwoWeekAveragePrecip<-mapply(Preciplag, tarssmall$siteid, tarssmall$date)
...
tempplot2016 <-ggplot(tars2016, aes(as.Date(date), as.numeric(TwoWeekAverageT)))+
