...
# specrich <- cbind(specrich, TwoWeekAvgMT, DegreeDays)
...
SpecRichLat<-ggplot(specrich,aes(lat2, num_species))+
...
SpecRichTemp<-ggplot(specrich, aes(TwoWeekAvgMT, num_species))+
...
CTTWAMT.mat<- mapply(Templag, tarssmall$siteid, tarssmall$date)
tarssmall$TwoWeekAvgMT <-c(unlist(CTTWAMT.mat[1,]))
tarssmall$DegreeDays <- c(unlist(CTTWAMT.mat[2,]))
...
tempplot2016 <-ggplot(tars2016, aes(as.Date(date), as.numeric(TwoWeekAvgMT)))+
  ...
  labs(x = "Date", y = "Two Week Average Maximum Temperature in C")+
  scale_x_date(labels = date_format("%m-%Y"), date_breaks = "1 month")+
