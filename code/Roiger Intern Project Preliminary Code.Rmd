...
taxonomy = read.csv("~/mosquito-intern/resources/mosquito_taxonomy.csv")
...
specrich <- ddply(idsmall, ~ uniquePlotIDs + collectDate + Year + HoursOfTrapping + Precipmm + Max.TempC, summarize, num_species = length(scientificName))

#Merging to get lat2 lon2 data
specrich <- merge(x = uniquetrap, y = specrich, by.y = "uniquePlotIDs", by.x = "uniquePlotIDs")

#Changin lat2 to a numeric
specrich$lat2<-as.numeric(specrich$lat2)

specrich[specrich$num_species>30,]

# specrich$collectDate<-
...
#Preliminary graph of Species Richness vs. maximum temp
ggplot(specrich, aes(Max.TempC, num_species))+
  geom_point(aes(colour = Year))+
  labs(x = "Maximum Temperature", y = "Number of Species")+
...
# ggplot(specrich, aes(as.Date(MonthDay), Max.TempC))+
#   geom_point(aes(colour = Year))+
#   labs(x = "Date of Collection", y = "Maximum Temperature")+
#   ggtitle("Maximum Temperature in Celsius vs. Date of Collection")
...
plot2016 <-ggplot(tars2016, aes(as.numeric(lat2), as.Date(date)))+
  geom_point(color = "darkblue")+
  labs(x = "Latitude", y = "Date")+
  scale_x_continuous(limits = c(30,65))+
  scale_y_date(labels = date_format("%m-%Y"), date_breaks = "1 month")+
  ggtitle("Date versus Latitude for 2016")
plot2014 <-ggplot(tars2014, aes(as.numeric(lat2), as.Date(date)))+
  geom_point(color = "pink")+
  scale_x_continuous(limits = c(30,65))+
  scale_y_date(labels = date_format("%m-%Y"), date_breaks = "1 month", limits = as.Date(c("2014-05-01","2014-11-01")))+
  labs(x = "Latitude", y = "Date")+
  ggtitle("Date versus Latitude for 2014")


tally(tarssmall$Year)
grid.arrange(plot2014, plot2016)

# gg + geom_polygon(data = tars.firstdomain, aes(lon2,lat2, fill = domainID))
