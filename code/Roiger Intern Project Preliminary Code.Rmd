...
#Merging trap df with sort df
trap<- merge(x = sort[,c("sampleID", "setDate", "subsampleID", "totalWeight", "subsampleWeight", "bycatchWeight")], y = trap, by.y = c("sampleID", "setDate"), by.x = c("sampleID", "setDate"), all.y = T)

#Creating a dataframe with only the unique plotIDs and lat2 lon2 data for merging
uniquetrap<-trap[!duplicated(trap[,7]),c("uniquePlotIDs","lat2","lon2")]
...
#Merge with trap so that id df contains samples where no mosquitos were found
id <- merge(x = trap[,c(1, 2:7,18,24,35,36)], y = id, by.y = c('subsampleID', 'lat2', 'lon2', 'uniquePlotIDs'), by.x = c('subsampleID', 'lat2', 'lon2', 'uniquePlotIDs'), all.x = T)
...
id$Date<-substr(id$setDate.x,1,10)
...
id$Timediff<-ifelse(substr(id$setDate.x,9,10)!=substr(id$collectDate,9,10), 24 - ((as.numeric(substr(id$setDate.x,15,16))/60)+(as.numeric(substr(id$setDate.x,12,13)))) + ((as.numeric(substr(id$collectDate,15,16))/60)+(as.numeric(substr(id$collectDate,12,13)))),((as.numeric(substr(id$collectDate[8],15,16))/60)+(as.numeric(substr(id$collectDate[8],12,13)))) - ((as.numeric(substr(id$setDate.x[8],15,16))/60)+(as.numeric(substr(id$setDate.x[8],12,13)))))
