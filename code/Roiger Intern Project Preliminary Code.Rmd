...
specrich <- ddply(id, ~ uniquePlotIDs + collectDate + Year + HoursOfTrapping + Precipmm + Max.TempC, summarize, num_species = length(scientificName))
...
#Preliminary graph of Species Richness 
...
ggplot(specrich2016, aes(as.Date(collectDate), num_species))+
  geom_point(aes(colour = num_species))+
...
ggplot(specrich, aes(lat2, Max.TempC)) +
  geom_point(aes(colour = num_species)) +
  labs(x = "Latitude", y = "Precipitation in mm") +
  ggtitle("Precipitation vs. Latitude Overall")
...
#From these graphs establish some sort of relationship between variables use to build models

#Do I talk about model building/ model selection or jump immediately to a model

#Layout assumptions for linear modeling
#LINE acronym

mod1 <- lm(num_species ~ lat2, data = specrich)
summary(mod1)
  
mod2 <- lm(num_species ~ lat2 + Year, data = specrich)
summary(mod2)

mod3 <- lm(num_species ~ lat2 + Max.TempC + Year, data = specrich)
summary(mod3)
vif(mod3)

#Deconstruction of appropriateness of linear model 
#Linearity: Examining the scatterplot of species richness vs. lat does not appear to be linear
#Independence: Sampling could be geographically related because of environments at plots, sites, and domains
#Normality: Normal Q-Q plot
#Equal Variance: Residuals Plot

plot(mod3)

#Conclusion: these relationships could exist but a different model is necessary
...
tars$lat2<- as.numeric(tars$lat2)
tars$lon2<- as.numeric(tars$lon2)
