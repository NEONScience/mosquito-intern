...
variables and collect all the information we need into certain dataframes.
To start, we will be extracting latitude and longitude information from NEON's
...
address this challenge, we use an if-else statement to find observations where
the set and collect date of the sample differed. We then converted the minutes
of each set and collect time into hours. Next we calculated the number of hours
from when the trap was set until midnight by subtracting the set time from 24, 
we then added the number of hours the trap was deployed on the collect day,
yielding the length of trap deployment. For observations where the set and 
collect date were the same, minutes were also converted to hours, and then
set time was subtracted from the collect time to produce the deployment time. 
...
factor. Next we extract the date of mosquito collection without the time, by 
using 'substr' again to take only the first ten characters of 'collectDate'. 
